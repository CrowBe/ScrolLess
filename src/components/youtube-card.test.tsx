...
describe('YouTubeCard', () => {
  it('renders the title', () => {
    render(<YouTubeCard item={base} onMarkRead={vi.fn()} />);
    expect(screen.getByText('Test Video')).toBeInTheDocument();
  });

  it('has unread class when unread', () => {
    const { container } = render(<YouTubeCard item={base} onMarkRead={vi.fn()} />);
    expect(container.querySelector('.card--unread')).toBeInTheDocument();
  });

  it('does not have unread class when read', () => {
    const { container } = render(<YouTubeCard item={{ ...base, is_read: true }} onMarkRead={vi.fn()} />);
  ...
  it('calls onMarkRead when expanding unread item', () => {
    ...
    render(<YouTubeCard item={base} onMarkRead={markRead} />);
    fireEvent.click(screen.getByRole('button'));
    expect(markRead).toHaveBeenCalledWith('youtube:abc');
  });

  it('thumbnail expands its container class when expanded', () => {
    const { container } = render(<YouTubeCard item={{ ...base, thumbnail_url: 'https://img.example.com/thumb.jpg' }} onMarkRead={vi.fn()} />);
