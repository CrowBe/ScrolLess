...
describe('XCard', () => {
  it('renders author handle', () => {
    render(<XCard item={{ ...base, author: '@testuser' }} onMarkRead={vi.fn()} onToggleSave={vi.fn()} />);
    expect(screen.getByText('@testuser')).toBeInTheDocument();
  });

  it('falls back to "Unknown" when no author', () => {
    render(<XCard item={base} onMarkRead={vi.fn()} onToggleSave={vi.fn()} />);
    expect(screen.getByText('Unknown')).toBeInTheDocument();
  });

  it('renders content_preview when available', () => {
    render(<XCard item={{ ...base, content_preview: 'This is a tweet.' }} onMarkRead={vi.fn()} onToggleSave={vi.fn()} />);
  ...
  it('truncates long content_preview to 180 chars when collapsed', () => {
    ...
    render(<XCard item={{ ...base, content_preview: longText }} onMarkRead={vi.fn()} onToggleSave={vi.fn()} />);
